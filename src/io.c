...
struct io_sock_s {
    ...
    enum {
		lstn, /* listener */
		o_conn, /* outbound connection */
        i_conn, /* inbound connection */
		tun /* tunnel */
	} typ;
    int alive;
...
static inline io_ctx_t * _new_io_ctx(int tun_fd, int epoll_fd) {
    ...
    ctx->tun_fd = tun_fd;
    if (shtab_init(&ctx->live_sockets, destroy_sock, "live_sock_tbl") != 0) {
        log_warn("io", L("Could not initialize live-sockets hash-table"));
        free(ctx);
    }
...
static inline void destroy_conn_sock_data(io_sock_t *sock) {
    assert(sock->typ == o_conn || sock->typ == i_conn);
